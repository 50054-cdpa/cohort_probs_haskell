...
import SUTD.Compiler.SimplyTypedLambdaCalculus hiding ((-))
...
eval (LambdaTerm x ty t) = return (LambdaValue (LambdaTerm x ty t))
eval (FixTerm t1) = do 
    v1 <- eval t1
    t1'' <- case v1 of 
        { LambdaValue (LambdaTerm f tf t1') -> appSubst (f, FixTerm t1) t1'
...
eval (AppTerm t1 t2) = do 
    v1 <- eval t1
    t3' <- case v1 of 
        { LambdaValue (LambdaTerm x ty t3) -> appSubst (x, t2) t3
