...
appSubst (x, u) (LetTerm y ty t2 t3)
    | (y /= x) && y `member` fv u = do
        t4 <- appSubst (x, u) t2
        t5 <- appSubst (x, u) t3
        return (LetTerm y ty t4 t5)
    | otherwise = do
        ...
        return (LetTerm z ty t4 t5)
appSubst (x, u) (LambdaTerm y ty t2)
    | (y /= x) && y `member` fv u = do
        t3 <- appSubst (x, u) t2
        return (LambdaTerm y ty t3)
    | otherwise = do
